...
- **Bedrock to Stride tasks.** Committed Pillar decisions decompose into Stride goals and tasks on a project board — **one goal per sprint** (see [`sprint-cadence.md`](sprint-cadence.md)) — each task enriched with key files, acceptance criteria, verification steps, and pitfalls drawn from the Bedrock. The Hands TPM (see [`personas.md`](personas.md)) owns this decomposition: all sprint goals are created during Sprint 0 (see [`sprint-0.md`](sprint-0.md)), but only the first sprint's goal is decomposed and enriched there — each later sprint decomposes just-in-time at its own boundary ceremony.
...
2. **Bedrock → tasks.** The TPM decomposes Pillar items into Stride goals and tasks via `stride:stride-workflow` and `stride:task-decomposer`, on the sprint cadence ([`sprint-cadence.md`](sprint-cadence.md)): **one goal per sprint**, all goals created at Sprint 0 (later ones sparse — outcome, deferred unknowns, task stubs), with each sprint's tasks decomposed and enriched (`stride:task-enricher` — `key_files`, `acceptance_criteria`, `verification_steps`, `pitfalls` drawn from the Bedrock) at that sprint's boundary ceremony, not in advance. A committed Pillar change that cascades into work reshapes the sprint plan the same way.
...
2. **Bedrock → tasks.** The TPM still plans on the sprint cadence — `sprint_plan.md` carries each sprint's section, and the active sprint's written task list uses the same enrichment shape (`key_files`, `acceptance_criteria`, `verification_steps`, `pitfalls`) drawn from the Bedrock, populated at that sprint's boundary ceremony. The format is preserved so that a later `stride` install can lift each sprint onto a board as one goal with minimal translation.
